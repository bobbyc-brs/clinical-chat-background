...
def run(args):
    ...
    if args.rpc_host and args.rpc_port:
        forwarder = RPCForwarder(args.rpc_host, args.rpc_port, args.rpc_method)
    else:
    ...
    with sd.InputStream(device=2, samplerate=SAMPLE_RATE, channels=CHANNELS, dtype="float32", blocksize=int(SAMPLE_RATE * BLOCK_SECONDS), callback=audio_buffer.callback):
...
def parse_args():
    ...
    p.add_argument("--device-id", default="doctor-device")
    p.add_argument("--speaker-role", choices=["patient", "doctor"], default="doctor")
